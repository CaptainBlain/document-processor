...
from auth_dp.target import Target
...
def get_services_directory():
    return str(get_current_path()) + '/services/'
  
def get_pdf_cell_json(target, thumbnail, pdfLink, issue):
  json = {}
...
def get_key(target):
  fileName = ''
  if target == Target.BestPractiseIndustry:
    fileName = os.path.join(get_services_directory(), 'best-practice-industry-account.json')
  elif target == Target.BuildingSolutionsUK:
    fileName = os.path.join(get_services_directory(), 'building-solutions-uk-account.json')
  elif target == Target.BuildingAndConstructionReview:
    fileName = os.path.join(get_services_directory(), 'buildingandconstructionreview-account.json')
  elif target == Target.BuildingAndFaciltiesNews:
    fileName = os.path.join(get_services_directory(), 'buildingandfacilitiesnews-account.json')
  elif target == Target.BusinessAndIndustryToday:
    fileName = os.path.join(get_services_directory(), 'bait-service-account.json')
  elif target == Target.FoodAndDrinkMatters:
    fileName = os.path.join(get_services_directory(), 'foodanddrinkmatters-account.json')
  elif target == Target.IndustrialProcessNews:
    fileName = os.path.join(get_services_directory(), 'industrialprocessnews-account.json')
  elif target == Target.ProductAndServicesReview:
    fileName = os.path.join(get_services_directory(), 'productservicesreview-account.json')
  elif target == Target.SolutionsPublishing:
    fileName = os.path.join(get_services_directory(), 'solutionspublishing-account.json')
  elif target == Target.TradexNews:
    fileName = os.path.join(get_services_directory(), 'tradexnews-account.json')
  elif target == Target.IndustryUpdate:
    fileName = os.path.join(get_services_directory(), 'industry-update-service-account.json')
  return fileName 
